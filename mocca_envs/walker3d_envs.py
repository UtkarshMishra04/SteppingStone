...
class Walker3DStepperEnv(EnvBase):
    ...
    def __init__(self, render=False):
        ...
        self.make_phantoms_yes = False #True
        self.phantoms = []
    ...
    def create_terrain(self):
        ...
        self.stoneId = self._p.createCollisionShape(pybullet.GEOM_MESH, fileName=os.path.join(current_dir, "data", "objects", "assets", "stone.obj"), meshScale=[3,3,10])

        self.boxHalfLength = 1.5
        self.boxHalfWidth = 2.5
        self.boxHalfHeight = 0.1
        self.segmentLength = 5

        self.colBoxId = self._p.createCollisionShape(pybullet.GEOM_BOX,
                                          halfExtents=[self.boxHalfLength, self.boxHalfWidth, self.boxHalfHeight])

        self.segmentStart = 0

        self.start_level = self._p.createMultiBody(baseMass=0,
                            baseCollisionShapeIndex=self.colBoxId,
                            basePosition=[self.segmentStart, 0, -0.1])

        self.segmentStart += 1.9
        ...
        for i in range(self.segmentLength):
          ...
          for j in range(self.width):
            self.stones_sim.append(self._p.createMultiBody(baseMass=0,
                              baseCollisionShapeIndex=self.stoneId,
                              basePosition=[self.segmentStart, 0.65 * (i % 2) + j - self.width / 2., -1.6 + (2*np.random.rand()-1)*0.2],
                              baseOrientation=self._p.getQuaternionFromEuler([(2*np.random.rand()-1)*np.pi/12, 0, 0])))
          self.segmentStart += 0.7

        self.segmentStart += 1.2

        self.finish_level = self._p.createMultiBody(baseMass=0,
                    baseCollisionShapeIndex=self.colBoxId,
                    basePosition=[self.segmentStart, 0, -0.1])
    ...
    def randomize_terrain(self):
        ...
        self._p.removeBody(self.start_level)
        for itr in range(len(self.stones_sim)):
            self._p.removeBody(self.stones_sim[itr])
        self._p.removeBody(self.finish_level)
